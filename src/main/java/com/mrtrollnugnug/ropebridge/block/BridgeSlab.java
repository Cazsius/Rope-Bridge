...
public class BridgeSlab extends Block {
	...
	public BridgeSlab(AxisAlignedBB bounds, int boundIndex) {
		...
		this.setDefaultState(
				this.blockState.getBaseState().withProperty(TYPE, EnumType.OAK).withProperty(ROTATED, false));
		this.bounds = bounds;
	}
	...
	public List<ItemStack> getDrops(IBlockAccess world, BlockPos pos, IBlockState state, int fortune) {
		...
		ret.add(new ItemStack(Blocks.WOODEN_SLAB, (int) Math.floor(ConfigurationHandler.getSlabsPerBlock() / 2),
				slabMeta));
	...
	public IBlockState getActualState(IBlockState state, IBlockAccess worldIn, BlockPos pos) {
		...
		return super.getActualState(state, worldIn, pos).withProperty(FRONT, frontAmount).withProperty(BACK,
				backAmount);
	}
