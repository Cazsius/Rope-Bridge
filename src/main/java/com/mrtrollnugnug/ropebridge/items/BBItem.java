...
public class BBItem extends Item
{
     ...
     * 
     * int x1,y1,x2,y2,z,z2; int Xdiff = Math.abs(pos1.getX()-pos2.getX()); int
     * Zdiff = Math.abs(pos1.getZ()-pos2.getZ()); boolean rotate; if (Xdiff >
     * Zdiff) { rotate = false; x1 = pos1.getX(); y1 = pos1.getY(); x2 =
     * pos2.getX(); y2 = pos2.getY(); z = pos1.getZ(); z2 = pos2.getZ(); } else
     * { rotate = true; x1 = pos1.getZ(); y1 = pos1.getY(); x2 = pos2.getZ(); y2
     * = pos2.getY(); z = pos1.getX(); z2 = pos2.getX(); } if (Math.abs(z2-z)>3)
     * {
     * tellPlayer("Sorry, bridge must be built in a cardinal dirrection. Please try again."
     * ); return; } double m; double b; double distance; int distInt;
     * 
     * m = (double)(y2-y1)/(double)(x2-x1); if (!Main.ignoreSlopeWarnings &&
     * Math.abs(m)>0.2) {
     * tellPlayer("Sorry, your slope is too great. Please try again."); return;
     * } b = (double)y1-(m*(double)x1); distance = Math.abs(x2-x1); distInt =
     * Math.abs(x2-x1);
     * 
     * // Check if bridge longer than 0 if (distInt < 2) { // bridge too short
     * return; }
     * 
     * // Check for materials in inventory if (!hasMaterials(distInt-1) &&
     * !player.capabilities.isCreativeMode) { return; }
     * 
     * for (int x = Math.min(x1, x2)+1; x<= Math.max(x1, x2)-1; x++) { for (int
     * y = Math.max(y1, y2); y>= Math.min(y1, y2)-distInt/8-1; y--) { double
     * funcVal = m*(double)x+b-(distance/1000)*(Math.sin((x-Math.min(x1,
     ...
     * 
     * 
     * if (allClear) { int type = inputType==-1 ? getWoodType() : inputType; if
     * (!player.capabilities.isCreativeMode) { takeMaterials(distInt-1); if
     ...
     * spawnSmoke(finPos, finTimes); } }, 1000); } }
     * 
     * private void buildBridge(LinkedList<SlabPos> bridge, int type) { SlabPos
     * slab; if(!bridge.isEmpty()) { slab = bridge.pop(); // Server call build x
     * y z Main.snw.sendToServer(new bridgeMessage(1, slab.x, slab.y, slab.z,
     * slab.level, (slab.rotate?1:0)+2*type));
     * 
     * spawnSmoke(new BlockPos(slab.x, slab.y, slab.z), 1); // play sound at x y
     * z wood Main.snw.sendToServer(new bridgeMessage(0, slab.x, slab.y, slab.z,
     * 1, 0));
     * 
     * final LinkedList<SlabPos> finBridge = bridge; final int finType = type;
     * buildTimer.schedule(new TimerTask() { public void run() {
                        ...
                        if (isBridgeBlock(currentBlockState.getBlock())) {
                            if (currentBlockState.getBlock().getMetaFromState(currentBlockState) == meta) {
                                ...
                                buildTimer.schedule(new TimerTask() {
                                    public void run()
                                    {
                                        breakBridge(worldInFinal, currentPosFinal, metaFinal);
                                    }
                                }, 100);
                            }
                        }
     ...
     * stringHad = 0;
     * 
     * for (int i = 0; i < 36; i++) { ItemStack stack =
     * player.inventory.mainInventory[i]; if (stack == null) { continue; }
     * String name = stack.getItem().getUnlocalizedName(); if
