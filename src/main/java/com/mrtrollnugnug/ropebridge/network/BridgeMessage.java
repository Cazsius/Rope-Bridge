	...
	public static class BridgeMessageHandler implements IMessageHandler<BridgeMessage, IMessage> {
		...
		public IMessage onMessage(BridgeMessage message, MessageContext ctx) {
			if (ctx.side == Side.SERVER) {
				...
				FMLCommonHandler.instance().getMinecraftServerInstance().addScheduledTask(() -> BridgeBuildingHandler
						.newBridge(player, player.getHeldItemMainhand(), -1, message.from, message.to));
			}
			return null;
		}
	}
