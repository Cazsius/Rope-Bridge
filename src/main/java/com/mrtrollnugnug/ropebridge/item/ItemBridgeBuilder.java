...
public class ItemBridgeBuilder extends ItemBuilder {
			...
			if (this.getMaxItemUseDuration(stack) - timeLeft > 10) {
				if (!player.onGround) {
					...
					if (hit.typeOfHit == Type.BLOCK) {
						final BlockPos floored = new BlockPos(Math.floor(player.posX), Math.floor(player.posY) - 1,
								Math.floor(player.posZ)).down();
	...
	public boolean onBlockStartBreak(ItemStack itemstack, BlockPos pos, EntityPlayer player) {
		...
		if (!player.world.isRemote && player.isSneaking()
				&& isBridgeBlock(player.world.getBlockState(pos).getBlock())) {
	...
	private static boolean isBridgeBlock(Block blockIn) {
		return blockIn == ContentHandler.blockBridgeSlab1 || blockIn == ContentHandler.blockBridgeSlab2
				|| blockIn == ContentHandler.blockBridgeSlab3 || blockIn == ContentHandler.blockBridgeSlab4;
	}
	 ...
	 */
	private static void breakBridge(final EntityPlayer player, final World worldIn, final BlockPos posIn,
			final int meta) {
						...
						for (int z = pos.getZ() - zRange; z <= pos.getZ() + zRange; z++) {
							...
							if ((x - pos.getX() == 0 && z - pos.getZ() == 0) || queue.contains(currentPos)) {
								...
								if (isBridgeBlock(currentBlockState.getBlock())
										&& currentBlockState.getBlock().getMetaFromState(currentBlockState) == meta) {
		...
		public void run() {
			...
			if (world.getBlockState(pos).getBlock() instanceof BridgeSlab) {
				FMLCommonHandler.instance().getMinecraftServerInstance()
						.addScheduledTask(() -> world.destroyBlock(pos, drop));
			}
			if (queue.isEmpty()) {
				timer.cancel();
			}
