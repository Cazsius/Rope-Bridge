...
public class Builder {
	public static void newBridge(EntityPlayer player, float playerFov, ItemStack stack, int inputType, BlockPos pos1, BlockPos pos2) {
		...
		if (!hasMaterials(player, distInt-1) && !player.capabilities.isCreativeMode) {
			return;
		}
	...
	private static void spawnSmoke(World world, BlockPos pos, int times) {
		if (times > 0) {
			...
			final World finworld = world; final BlockPos finPos = pos; final int finTimes = times-1;
		    new Timer().schedule(new TimerTask() { public void run() { spawnSmoke(finworld, finPos, finTimes); } }, 1000);
		}
	}
